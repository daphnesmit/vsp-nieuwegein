...
export type HomePageTemplateQueryVariables = {};


export type HomePageTemplateQuery = { markdownRemark: Maybe<{ frontmatter: Maybe<(
      Pick<MarkdownRemarkFrontmatter, 'title'>
      & { image: Maybe<{ childImageSharp: Maybe<{ fluid: Maybe<GatsbyImageSharpFluidFragment> }> }> }
    )> }> };

export type RecentPageTemplateQueryVariables = {};


export type RecentPageTemplateQuery = { markdownRemark: Maybe<{ frontmatter: Maybe<Pick<MarkdownRemarkFrontmatter, 'title'>> }>, allMarkdownRemark: { edges: Array<{ node: (
        Pick<MarkdownRemark, 'id' | 'excerpt'>
        & { fields: Maybe<Pick<MarkdownRemarkFields, 'slug'>>, frontmatter: Maybe<Pick<MarkdownRemarkFrontmatter, 'date' | 'title' | 'templateKey'>> }
      ) }> } };

export type RecentPostByIdQueryVariables = {
  id: Scalars['String']
};


export type RecentPostByIdQuery = { markdownRemark: Maybe<(
    Pick<MarkdownRemark, 'id' | 'html'>
    & { frontmatter: Maybe<Pick<MarkdownRemarkFrontmatter, 'date' | 'title' | 'description'>> }
  )> };
